...
public class CommandRegistration {
    ...
    private static net.minecraft.network.chat.MutableComponent withSuggestCommandStyle(net.minecraft.network.chat.Component component, String suggestCommand) {
        return component.copy().withStyle(s -> s.withColor(net.minecraft.ChatFormatting.RED)
    ...
    private static void registerReviveCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("revive")
            ...
            .executes(context -> {
                context.getSource().sendFailure(withSuggestCommandStyle(MessageUtil.get("usage-revive"), "/revive "));
                return 0;
            })
            .then(Commands.argument(PLAYER, StringArgumentType.word())
                .suggests((context, builder) -> SharedSuggestionProvider.suggest(
    ...
    private static void registerSetLivesCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("psetlives")
            .requires(source -> source.hasPermission(2))
            .executes(context -> {
                context.getSource().sendFailure(withSuggestCommandStyle(MessageUtil.get("usage-psetlives"), "/psetlives "));
                return 0;
            })
            .then(Commands.argument(PLAYER, StringArgumentType.word())
                .suggests((context, builder) -> SharedSuggestionProvider.suggest(
                ...
                .executes(context -> {
                    String targetName = StringArgumentType.getString(context, PLAYER);
                    context.getSource().sendFailure(withSuggestCommandStyle(MessageUtil.get("usage-psetlives-player", PLAYER, targetName), "/psetlives " + targetName + " "));
                    return 0;
                })
