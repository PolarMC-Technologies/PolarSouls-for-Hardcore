...
public class CommandRegistration {
    private static Component styledComponent(String key, String command, Object... args) {
        return MessageUtil.get(key, args).copy().withStyle(s -> s.withColor(net.minecraft.ChatFormatting.RED)
    ...
    private static void registerReviveCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("revive")
            ...
            .executes(context -> {
                context.getSource().sendFailure(styledComponent("usage-revive", "/revive "));
                return 0;
            })
            .then(Commands.argument(PLAYER, StringArgumentType.word())
                .suggests((context, builder) -> SharedSuggestionProvider.suggest(
    ...
    private static void executeRevive(String targetName, CommandSourceStack source) {
        ...
        org.ssoggy.ssoggysouls.util.ConfigManager.ModConfig config = org.ssoggy.ssoggysouls.util.ConfigManager.getConfig();
        int defaultLives = config != null ? config.getDefaultLives() : 1;
    ...
    private static void registerSetLivesCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("psetlives")
            .requires(source -> source.hasPermission(2))
            .executes(context -> {
                context.getSource().sendFailure(styledComponent("usage-psetlives", "/psetlives "));
                return 0;
            })
            .then(Commands.argument(PLAYER, StringArgumentType.word())
                .suggests((context, builder) -> SharedSuggestionProvider.suggest(
                ...
                .executes(context -> {
                    String targetName = StringArgumentType.getString(context, PLAYER);
                    context.getSource().sendFailure(styledComponent("usage-psetlives-player", "/psetlives " + targetName + " ", PLAYER, targetName));
                    return 0;
                })
