...
public class CommandRegistration {
    ...
    private static void registerReviveCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("revive")
            ...
            .executes(context -> {
                context.getSource().sendFailure(net.minecraft.network.chat.Component.empty().append(MessageUtil.get("usage-revive")
    ...
    private static void registerSetLivesCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("psetlives")
            .requires(source -> source.hasPermission(2))
            .executes(context -> {
                context.getSource().sendFailure(net.minecraft.network.chat.Component.empty().append(MessageUtil.get("usage-psetlives")
                    ).withStyle(s -> s.withColor(net.minecraft.ChatFormatting.RED)
            ...
            .then(Commands.argument(PLAYER, StringArgumentType.word())
                ...
                .executes(context -> {
                    String targetName = StringArgumentType.getString(context, PLAYER);
                    context.getSource().sendFailure(net.minecraft.network.chat.Component.empty().append(MessageUtil.get("usage-psetlives-player", PLAYER, targetName)
                        ).withStyle(s -> s.withColor(net.minecraft.ChatFormatting.RED)
